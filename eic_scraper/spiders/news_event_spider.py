...
class  NewsEventSpider(scrapy.Spider):
    ...
    def parse_news_event_detail(self, response):
        ...
        url = response.url
        published = item.xpath('.//time[@itemprop="datePublished"]/text()').get().strip()
        content = remove_empty_list_item(item.xpath('.//p/descendant-or-self::*/text()').getall());
    ...
    def parse_incentive_packages(self, response):
        ...
        package_headings = remove_empty_list_item(heading_container.xpath('.//li/descendant-or-self::*/text()').getall())
        ...
        for index_body, body in enumerate(package_bodies):
            ...
            if readmore is not None:
                ...
                for index_incentive, incentive in enumerate(incentives):
                    ...
                    item = IncentiveItem()
                    item['name'] = remove_empty_list_item(item_fields[0].xpath('.//descendant-or-self::*/text()').getall())
                    item['description'] = remove_empty_list_item(item_fields[1].xpath('.//descendant-or-self::*/text()').getall())
                    item['legal_reference'] = remove_empty_list_item(item_fields[2].xpath('.//descendant-or-self::*/text()').getall())
                    item['law_section'] = remove_empty_list_item(item_fields[3].xpath('.//descendant-or-self::*/text()').getall())
                    item['sector'] = remove_empty_list_item(item_fields[4].xpath('.//descendant-or-self::*/text()').getall())
                    item['eligebility'] = remove_empty_list_item(item_fields[5].xpath('.//descendant-or-self::*/text()').getall())
                    item['rewarding_authority'] = remove_empty_list_item(item_fields[6].xpath('.//descendant-or-self::*/text()').getall())
                    item['implementing_authority'] = remove_empty_list_item(item_fields[7].xpath('.//descendant-or-self::*/text()').getall())
    ...
    def parse_incentives(self, response):
        ...
        for incentive in incentives_container.xpath('.//tbody/tr'):
            ...
            item['rewarding_authority'] = remove_empty_list_item(item_fields[6].xpath('.//descendant-or-self::*/text()').getall())
            item['implementing_authority'] = remove_empty_list_item(item_fields[7].xpath('.//descendant-or-self::*/text()').getall())
            item['incentive_package'] = response.xpath('//h1[@class="article-title"]/text()').get().strip()
    ...
    def parse_sector_leather(self, response):
        ...
        for header_index, header in enumerate(section_headers):
            ...
            if header_index == 0:
                ...
                organisations = header.xpath('.//following-sibling::p/strong/parent::p')
                organisation_address_map = {}
                for organisation in organisations:
                    address = remove_empty_list_item(organisation.xpath('.//following-sibling::p[1]/descendant-or-self::*[not(self::script)]/text()').getall())
                    organisation_name = organisation.xpath('.//text()').get().strip()
        ...
        sector_quotes = parse_sector_quotes(response.css('.sprocket-quotes-text'))
        if sector_quotes:
            ...
            if header_index == 4:
                ...
                for organisation_index, organisation in enumerate(organisations):
                    ...
                    if organisation_index == 1:
                        paragraphs = organisation.xpath('.//following-sibling::p[1] | .//following-sibling::p[2]')
                        address = remove_empty_list_item(paragraphs.xpath('.//descendant-or-self::*[not(self::script)]/text()').getall())
                    else:
                        address = remove_empty_list_item(organisation.xpath('.//following-sibling::p[1]/descendant-or-self::*[not(self::script)]/text()').getall())
    ...
    def parse_sector_horticulture(self, response):
        sector_content = {}
        section_headers = response.xpath('//h3[1] | //h3[2] | //h3[3] | //h3[4]')
        section_headers.append(section_headers.xpath('.//following-sibling::p/strong/parent::p')[0])

        for header_index, header in enumerate(section_headers):
            content = None
            if header_index == 2:
                content = parse_sector_quotes(header.xpath('.//following-sibling::table/tbody/tr/td'))
            elif header_index == 3:
                organisations = header.xpath('.//following-sibling::p/strong/parent::p')
                organisation_address_map = {}
                for organisation_index, organisation in enumerate(organisations):
                ...
                content = {}
                opportunity_headers = header.xpath('.//following-sibling::p/strong/parent::p')[:3]
                for opportunity_header in opportunity_headers:
                    opportunity = remove_empty_list_item(opportunity_header.xpath('.//following-sibling::ul[1]/li/text()').getall())
                    opportunity_name = opportunity_header.xpath('.//text()').get().strip()
    ...
    def parse_sector_argriculture(self, response):
        sector_content = remove_empty_list_item(response.css('.article-content').xpath('.//ul/li/text()').getall())
    ...
    def parse_sector_manufacturing(self, response):
        ...
        for header_index, header in enumerate(section_headers):
            content = None
            if header_index == 0:
                content = remove_empty_list_item(header.xpath('.//following-sibling::ul[1]/li/text() | .//following-sibling::ul[2]/li/text() | .//following-sibling::ul[3]/li/text()').getall())
            else:
                content = remove_empty_list_item(header.xpath('.//following-sibling::ul[1]/li/text()').getall())
    ...
    def parse_sector_pharmaceuticals(self, response):
        ...
        for header_index, header in enumerate(section_headers):
            content = None
            if header_index == 0:
                content = remove_empty_list_item(header.xpath('.//following-sibling::p[1]/text()').getall())
            else:
                content = remove_empty_list_item(header.xpath('.//following-sibling::ul[1]/li/text()').getall())
    ...
    def parse_sector_power(self, response):
        ...
        for header_index, header in enumerate(section_headers):
            content = None
            if header_index == 0:
                content = remove_empty_list_item(header.xpath('.//following-sibling::p/text()').getall())
            else:
                content = remove_empty_list_item(header.xpath('.//following-sibling::ul[1]/li/text()').getall())
...
def parse_sector_quotes(quotes_container):
    ...
    if quotes_container:
        for quote_container in quotes_container:
            text = remove_empty_list_item(quote_container.xpath('.//descendant-or-self::*[not(self::img)]/text()').getall())
            images = quote_container.xpath('.//descendant-or-self::img/@src').getall()
